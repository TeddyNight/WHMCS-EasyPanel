...
function easypanel_ConfigOptions() {
	...
	# Should return an array of the module options for each product - 
maximum of 24

    $configarray = array(
		 "CDN"				=> array("Type" => 
"yesno", "Description" => "CDN网站"),				//1
		 "空间类型"			=> array("Type" => "text", 
"Size" => "5"),							//2
		 "web配额" 			=> array( "Type" => 
"text", "Size" => "5", "Description" => "MB" ),	//3
		 "数据库类型"			=> array( "Type" => 
"dropdown", "Options" => "mysql,sqlsrv"),		//4
		 "数据库配额" 		=> array( "Type" => "text", "Size" 
=> "5", "Description" => "MB" ),	//5
		 "FTP" 				=> array( "Type" => 
"yesno", "Description" => "是否允许ftp" ),		//6
		 "独立日志" 			=> array( "Type" => 
"yesno", "Description" => "是否开启独立日志" ),	//7
	     "绑定域名数" 		=> array( "Type" => "text", "Size" 
=> "5"),							//8
	     "连接数"			=> array( "Type" => "text", "Size" 
=> "5"),							//9
	     "带宽限制"			=> array( "Type" => "text", "Size" 
=> "5","Description" => "K/S"),	//10
		 "默认绑定到子目录" 	=> array( "Type" => 
"text","Size"=>"20"),							
//11
		 "允许绑定子目录" 		=> array( "Type" => 
"yesno", "Description" => "是否允许绑定域名到子目录"),//12
		 "最多绑定子目录数" 	=> array( "Type" => 
"text","Size"=>"5"),							
//13
    	 "流量限制"			=> array( "Type" => 
"text","Size"=>"5","Description" => "G/月"),	//14
    	 "管理变量"			=> array( "Type" => 
"text","Size"=>"12"),							
//15
    	 "工作数"			=> array( "Type" => 
...
function easypanel_update_account($params,$edit)
{
	...
	$whmCall->addParam('templete',$params["configoption2"]);
        $quota = null;
        if (isset($params['configoptions']['quota'])) {
                 $quota = $params['configoptions']['quota'];
        } else if(!empty($params['configoption3'])) {
                 $quota = $params["configoption3"];
        }
    $whmCall->addParam('web_quota',$quota);
	$whmCall->addParam('db_type',$params["configoption4"]);
        $db_quota = null;
        if (isset($params['configoptions']['db_quota'])) {
                 $db_quota = $params['configoptions']['db_quota'];
        } else if(!empty($params['configoption5'])) {
                 $db_quota = $params["configoption5"];
        }    
    $whmCall->addParam('db_quota',$db_quota);
    $whmCall->addParam('ftp',($params["configoption6"]== 'on'?1:0));
	$whmCall->addParam('log_file',($params["configoption7"]== 
'on'?1:0));  
    $whmCall->addParam('domain',$params['configoption8']);
    $max_cnn = null;
        if (isset($params['configoptions']['max_connect'])) {
                 $max_cnn = $params['configoptions']['max_connect'];
        } else if(!empty($params['configoption9'])) {
                 $max_cnn = $params['configoption9'];
        }    
    $whmCall->addParam('max_connect',$max_cnn);
    $whmCall->addParam('speed_limit',intval($params["configoption10"])*1);	
	$whmCall->addParam('subdir',$params["configoption11"]);
	$whmCall->addParam('subdir_flag',($params["configoption12"]== 
'on'?1:0));
	$whmCall->addParam('max_subdir',$params["configoption13"]);
        ...
        } else if(!empty($params['configoption14'])) {
                 $traffic = $params['configoption14'];
        }
	$whmCall->addParam('flow_limit',$traffic);
...
function easypanel_ClientArea($params) {
    ...
    # Output can be returned like this, or defined via a clientarea.tpl 
template file (see docs for more info)

	$code = '<form 
action="http://'.$params["serverip"].':3312/vhost/?c=session&a=login" 
...
function easypanel_AdminLink($params) {
	...
	$code = '<form 
action="http://'.$params["serverip"].':3312/admin/?c=session&a=login" 
method="post" target="_blank">
...
function easypanel_LoginLink($params) {
	...
	echo "<a 
href=\"http://".$params["serverip"].":3312/vhost/?username=".$params["username"]."\" 
target=\"_blank\" style=\"color:#cc0000\">登录easypanel</a>";
...
function easypanel_UsageUpdate($params) {
	...
	# Run connection to retrieve usage for all domains/accounts on 
$serverid
...
function easypanel_AdminServicesTabFields($params) {
    ...
    $result = 
select_query("mod_customtable","",array("serviceid"=>$params['serviceid']));
    ...
    $fieldsarray = array(
     'Field 1' => '<input type="text" name="modulefields[0]" size="30" 
value="'.$var1.'" />',
     'Field 2' => '<select 
name="modulefields[1]"><option>Val1</option</select>',
     'Field 3' => '<textarea name="modulefields[2]" rows="2" 
cols="80">'.$var3.'</textarea>',
